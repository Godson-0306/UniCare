...
function averageDispensingTime(prescriptions: Prescription[]) {
  ...
  const average = Math.round(
    completed.reduce((total, item) => total + minutesBetween(item.created_at, item.dispensed_at ?? item.created_at), 0) /
      completed.length
  );
      ...
      <div className="space-y-5">
        <header className="flex flex-col gap-3 border-b border-[var(--border)] pb-5 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <h2 className="font-display text-2xl font-semibold text-slate-950">Prescriptions awaiting dispensing</h2>
            <p className="text-sm text-[var(--muted)]">Queue management only. Open a prescription to dispense medication.</p>
          </div>
        ...
        <QueueMetricsRow
          items={[
            ...
            { label: "Dispensed today", value: dispensedToday(prescriptions).toString() },
            { label: "Waiting patients", value: pendingCount.toString() },
            { label: "Avg dispense time", value: averageDispensingTime(prescriptions) },
          ]}
        />
        ...
        <QueueFiltersBar>
          <div className="relative">
            ...
            <Input
              className="pl-9"
              placeholder="Search by prescription, name, or matric number"
              value={query}
        ...
        {loading ? <p className="rounded-md border border-[var(--border)] bg-white p-4 text-sm text-slate-500">Loading pharmacy queue...</p> : null}
          ...
          <QueueList>
            {filteredPrescriptions.map((prescription) => (
              <QueueListRow
                ...
                subtitle={`${prescription.student_name} · ${prescription.matric_number}`}
                badge={<Badge variant={statusVariant(prescription.status)}>{prescription.status.replaceAll("_", " ")}</Badge>}
                meta={[
                  { label: "Visit", value: prescription.visit_number || "N/A" },
                  { label: "Doctor", value: prescription.prescribed_by || "Clinical team" },
                  { label: "Prescribed", value: formatDateTime(prescription.created_at) },
                  { label: "Meds", value: `${prescription.items.length}` },
                ]}
                actions={
                  <>
                    <Button asChild size="sm">
                      <Link href={`/hospital/pharmacy/prescription/${prescription.id}`}>Open</Link>
                    </Button>
                    <Button asChild variant="outline" size="sm">
                      <Link href={`/hospital/pharmacy/history/${prescription.student_id}`}>
