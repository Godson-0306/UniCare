...
export default function PrescriptionCard({ item }: { item: Record<string, any> }) {
  ...
  const status = (item.status ?? item.state ?? "active").toString();
    ...
    <Card>
      <CardHeader className="flex items-start justify-between p-4">
        <div>
          ...
          <CardDescription className="text-xs text-slate-500">{dosage} • {frequency} • {duration}</CardDescription>
        </div>
